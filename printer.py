...
class prettyXarray (printerInterface):
    def __init__(self, value: gdb.Value):
        ...
        if(str(self.value.type) == 'xt::xarray'):
            self.shape = fetch_array(self.value['m_shape']['m_begin'], self.value['m_shape']['m_end'])
    ...
    def children(self):
        if(self.shape.size == 0):
            return
        order = "F" if str(self.value['m_layout']) == "xt::layout_type::column_major" else "C" # F like fortran style, for collumn major
